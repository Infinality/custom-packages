...
Freetype normally always honors the GETINFO selector for symmetric smoothing as long as the rendering mode isn't FT_LOAD_TARGET_MONO.  This patch enriches the behavior to inspect the font's gasp table to check ppem sizes and smoothing settings to better respect the font's support and disables it when appropriate.  This has the effect of fixing the rendering of Microsoft fonts at certain smaller point sizes, enabling grid fitting, and making it render glyph shapes identically to DirectWrite.

The patch was developed with the help of ChatGPT and is available in the freetype directory.  I'm attempting to upstream this, with "1" as the default, but in my patch, "2" is the default.  What follows here is the patch description generated by ChatGPT:

=====

This patch adds a `truetype:symmetric-rendering` FreeType property that controls the ClearType symmetric-rendering state reported to TrueType bytecode through `GETINFO` bit 18.

This matters because some modern TrueType fonts use that state to select different hinting instructions. In particular, fonts such as Segoe UI and Calibri can relax vertical grid fitting when symmetric rendering is reported, producing softer horizontal stems at small UI sizes.

The patch provides three modes:

```text
0 = Disable symmetric rendering
1 = Enable symmetric rendering (stock FreeType behavior)
2 = Auto-select using the font's version-1 `gasp` table
```

The `auto` mode uses the current PPEM and the font's `GASP_SYMMETRIC_GRIDFIT` / `GASP_SYMMETRIC_SMOOTHING` flags. When the font requests symmetric grid fitting without symmetric smoothing, GETINFO bit 18 is suppressed. Otherwise normal FreeType behavior is retained.

This produces substantially more Windows/DirectWrite-like TrueType hinting in tested fonts while preserving font-authored PPEM transitions. For example, Segoe UI retains strongly grid-fitted horizontal stems through its 19-PPEM `gasp` range, while Arial transitions at its own 17-PPEM boundary without the malformed glyphs caused by globally disabling symmetric rendering.

### Usage

Set the property through `FREETYPE_PROPERTIES`:

```bash
FREETYPE_PROPERTIES="truetype:symmetric-rendering=0"
```

Always disable symmetric-rendering reporting.

```bash
FREETYPE_PROPERTIES="truetype:symmetric-rendering=1"
```

Use traditional FreeType behavior.

```bash
FREETYPE_PROPERTIES="truetype:symmetric-rendering=2"
```

Use the `gasp`-aware automatic mode. **This is the default setting.**

The property can be combined with other FreeType properties, for example:

```bash
FREETYPE_PROPERTIES="truetype:interpreter-version=40 truetype:symmetric-rendering=2"
```

Applications must actually use the patched FreeType library for the setting to have an effect. Applications that bundle or use their own font rasterizer, including some Chromium-based applications, may ignore the system `FREETYPE_PROPERTIES` setting.

The patch does not modify fonts, LCD filtering, or the rasterizer itself. It changes only the rasterizer capability reported to TrueType hinting programs, allowing fonts to choose the hinting path appropriate to their own `gasp` settings and current PPEM.
